...
import { addTaskTC, fetchTasksTC, removeTaskAC, removeTaskTC } from "./state/tasks-reducer";
import {
...
const ToDoList = React.memo((props: PropsType) => {
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    accept: "task",
    drop: (item: any) => moveTask(item.task.id, item.todolistId),
    collect: (monitor) => ({
      isOver: !!monitor.isOver(),
    }),
  }));
  const moveTask = (task: TaskType, todolistId: string) => {
    const thunk = removeTaskTC({taskId: task.id, todolistId});
    thunk(dispatch);
    const thunk1 = addTaskTC(title: task.title, todolistId: props.todolist.id)
  ...
  return (
    <div className='todo-list' ref={drag}>
      <div className='title'>
        <h3>
          <EditableSpan
